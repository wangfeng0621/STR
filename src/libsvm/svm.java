...
import cn.edu.swufe.MatrixPredict;
...
abstract class QMatrix {
	abstract float[] get_Q(int column, int len);  //求Q矩阵的列数
...
abstract class Kernel extends QMatrix {
					...
					svm_parameter param)
	{
		switch(param.kernel_type)
		{
			...
			{
				double sum = 0;
				//System.out.println("预测程序进入了BRF核！");
				int xlen = x.length;
				int ylen = y.length;
				int i = 0;
				int j = 0;
				...
				//System.out.println("BRF核运算的结果:"+Math.exp(-param.gamma*sum));
...
class SVR_Q extends Kernel
{
	...
	SVR_Q(svm_problem prob, svm_parameter param)
	{
		...
		next_buffer = 0;
		System.out.println("我是svr我现在libsnm.svm.java文件的函数SVR_Q中");
		System.out.println("我是svr我现在在核函数中");
	}
...
public class svm {
	...
	private static void solve_epsilon_svr(svm_problem prob, svm_parameter param,
					double[] alpha, Solver.SolutionInfo si) 
 {
		System.out.println("我是svr现在在libsnm.svm.java文件的函数sovle_epsilon_svr中，现在开始写vector.txt文件！");
		// ****************************************
		try {
			File f = new File("..\\vector.txt");
			if (f.exists())
				f.delete();
			FileWriter output;
			output = new FileWriter("..\\vector.txt");
		...
		// ****************************************
			...
			for (i = 0; i < l; i++) {
				alpha2[i] = 0;
				linear_term[i] = param.beta * (param.p - prob.y[i]);
				y[i] = 1;
				alpha2[i + l] = 0;
				linear_term[i + l] = param.beta * (param.p + prob.y[i]);
				y[i + l] = -1;
			}

			Solver s = new Solver();			
			s.Solve(2 * l, new SVR_Q(prob, param), linear_term, y, alpha2,
					param.C, param.C, param.eps, si, param.shrinking);

			System.out.println("obj:"+si.obj);
			double sum_alpha = 0;
			for (i = 0; i < l; i++) {
			...
			// ****************************************
			for(i = 1; i <= prob.x[0].length; i++){
				double res = 0;
				for(int j = 0; j < l; j++){
					for(int k = 0; k < prob.x[j].length; k ++){
						if (i == prob.x[j][k].index){
							res = res + alpha[j]*prob.x[j][k].value;

						}
					}	
					//System.out.println("一个样本！");
				}
				res = res / param.beta;
				//System.out.println("一个向量的新值："+res);
				bw.write(Integer.toString(i) + ":" + Double.toString(res) +" ");
			}
			// ****************************************
	...
	private static double svm_svr_probability(svm_problem prob, svm_parameter param)
	{
		...
		svm_parameter newparam = (svm_parameter)param.clone();//完全克隆  因为序列化了
	...
	{	
		...
		try{
			BufferedReader fp;
			fp = new BufferedReader(new FileReader("..\\bata.txt"));
			String line;
			line = fp.readLine();			
//			StringTokenizer st = new StringTokenizer(line, " \t\n\r\f:");
//			st.nextToken();
			bata = Double.valueOf(line);	
			String line1;
			line1 = fp.readLine();
			b = Double.valueOf(line1);
			fp.close();
		}catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		   ...
		   model.param.svm_type == svm_parameter.NU_SVR)
		{
			...
			double sum = 0;
			//System.out.println(sv_coef.length);2643
			//System.out.println(model.SV[0].length);18
			//System.out.println(model.SV.length);2643
			
			
			//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			//for(int k =0;k<model.SV[0].length;k++ ){
			//	double resd = 0;
			//	for (int kk = 0;kk<sv_coef.length;kk++){
			//		resd = resd + sv_coef[kk]*model.SV[kk][k].value;
			//	}
			...
			for(i=0;i<model.l;i++)
				//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				//System.out.print("alpha的值："+sv_coef[i]);
				sum += sv_coef[i] * Kernel.k_function(x,model.SV[i],model.param);
			//System.out.println("sv_coef[i]:"+sv_coef[i]);
			//System.out.println("sum:"+sum);
			//下面这个对应着我们推导的公式
			sum = sum / bata;
			//%%%%%%%%%%%%%%%%%%%%
			//System.out.println("sum:"+sum);
			//System.out.println("rho:"+model.rho[0]);
			//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			//下面这句修改把他原来的model.rho[0]修改成了我们的b值，因为我们对model.rho[0]有一个model.rho[0]/beta的处理
			sum = sum - model.rho[0]/ bata;
			dec_values[0] = sum;
